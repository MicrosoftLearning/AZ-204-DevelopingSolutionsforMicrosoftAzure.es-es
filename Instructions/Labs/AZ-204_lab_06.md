...
En este laboratorio, registrarás una aplicación en Microsoft Entra ID, agregarás un usuario y luego probarás el acceso del usuario a la aplicación para validar que Microsoft Entra ID puede protegerla. También usarás la herramienta Probador de Graph para compilar y probar solicitudes en Graph API de una cuenta de usuario de Entra ID.
...
#### Tarea 2: Registrar una aplicación en el directorio de Entra ID

1. En el Azure Portal, usa el cuadro de texto **Buscar recursos, servicios y documentos** para buscar **Entra ID** y, en la lista de resultados, selecciona **Microsoft Entra ID**.

    > **Nota**: Esto redirige la sesión del explorador a la hoja del inquilino de Microsoft Entra ID asociado a la suscripción de Azure.

1. En la hoja de  **Microsoft Entra ID** , selecciona **Registros de aplicaciones** en la sección **Administrar**.
...
1. Ejecute los siguientes comandos para crear una nueva aplicación web de .NET Core basada en la plantilla de Modelo-Vista-Controlador (MVC) (reemplace los marcadores de posición `<application_ID>`, `<tenant_ID>` y `<domain_Name>` por los valores correspondientes que registró anteriormente en este laboratorio):
    ...
    > **Nota**: Si se produce un error en el terminal, es posible que uses PowerShell. En ese caso, quita las marcas `/S /Q` y vuelve a ejecutar el comando.
