...
1. En el **panel de Cloud Shell**, revise la lista de proveedores registrados actualmente. Observe que el proveedor **Microsoft.EventGrid** está incluido actualmente en la lista de proveedores.
...
1. Ejecute el comando siguiente para importar la versión 4.10.0 de **Azure.Messaging.EventGrid** desde NuGet:
    ...
    > **Nota**: El comando **dotnet add package** agregará el paquete **Microsoft.ApplicationInsights** desde NuGet. Para más información, vaya a [Azure.Messaging.EventGrid](https://www.nuget.org/packages/Azure.Messaging.EventGrid/4.10.0).

    > Ejecute el siguiente comando para crear la aplicación web de .NET: Seleccione **Cerrar el terminal** o el icono de la **papelera de reciclaje** para cerrar el terminal abierto y todos los procesos asociados.


1. Tarea 2: Modificar la clase Program para conectarse a Event Grid
...
#### <a name="task-2-modify-the-program-class-to-connect-to-event-grid"></a>En la pestaña del editor de código del archivo  **Program.cs** , elimine todo el código del archivo existente.
...
1. Tarea 3: Publicación de nuevos eventos
...
1. Agregue la siguiente línea de código para crear una nueva variable denominada **endpoint** de tipo **URI** mediante la constante de cadena **topicEndpoint** como parámetro de constructor:
    ...
    Revise el método **Main**, que ahora debe incluir: Guarde el archivo **Program.cs** .
...
1. **Nota**: Si hay algún error de compilación, revise el archivo **Program.cs** en la carpeta **Allfiles (F):\\Allfiles\\Labs\\09\\Solution\\EventPublisher**.
...
1. Observe la salida del mensaje de operación correcta de la aplicación de consola que se está ejecutando actualmente.
    ...
    > Seleccione **Cerrar el terminal** o el icono de la **papelera de reciclaje** para cerrar el terminal abierto y todos los procesos asociados.

1. Tarea 4: Observación de los eventos publicados

1. Vuelva a la ventana del explorador con la aplicación web **Visor de Azure Event Grid**.

#### <a name="task-4-observe-published-events"></a>Revise los eventos **Employees.Registration.New** creados por la aplicación de consola.
...
1. En este ejercicio, ha publicado nuevos eventos en el tema de Event Grid mediante una aplicación de consola de .NET.

#### <a name="review"></a>Ejercicio 4: Limpieza de la suscripción

Tarea 1: Apertura de Azure Cloud Shell

### <a name="exercise-4-clean-up-your-subscription"></a>En Azure Portal, seleccione el icono **Cloud Shell**, ![icono Cloud Shell](./media/az204_lab_CloudShell.png), para abrir una nueva sesión de Bash.

#### <a name="task-1-open-azure-cloud-shell"></a>Si el valor predeterminado de Cloud Shell es una sesión de PowerShell, seleccione **PowerShell** y, en el menú desplegable, seleccione **Bash**.

1. **Nota**: Si es la primera vez que inicia **Cloud Shell**, cuando se le pida que seleccione **Bash** o **PowerShell**, seleccione **PowerShell**. Si aparece el mensaje **No tiene ningún almacenamiento montado**, seleccione la suscripción que utiliza en este laboratorio y seleccione **Crear almacenamiento**.

    > Tarea 2: Eliminación de un grupo de recursos En el panel de **Cloud Shell**, ejecute el comando siguiente para eliminar el grupo de recursos **PubSubEvents**:

#### <a name="task-2-delete-a-resource-group"></a>**Nota**: El comando se ejecuta de forma asincrónica (según determina el parámetro *--no-wait*). Aunque podrá ejecutar otro comando de la CLI de Azure inmediatamente después en la misma sesión de Bash, los grupos de recursos tardarán unos minutos en quitarse.
...
#### <a name="task-3-close-the-active-applications"></a>Cierre la aplicación que ejecuta Visual Studio Code actualmente.

1. Revisar

1. En este ejercicio, ha limpiado la suscripción mediante la eliminación de los grupos de recursos usados en este laboratorio.
