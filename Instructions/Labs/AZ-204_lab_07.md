...
1. En la hoja **Crear almacén de claves**, en la pestaña **Aspectos básicos**, realice las siguientes acciones y seleccione la pestaña **Configuración de acceso**:
...
1. En la pestaña **Configuración de acceso**, cambie el modelo de permisos a **Directiva de acceso a almacenes**.

1. Seleccione la pestaña **Revisar y crear**, revise las opciones que seleccionó durante los pasos anteriores.
...
1. Ejecute el siguiente comando para usar **Azure Functions Core Tools** para crear un nuevo proyecto de Functions local en el directorio actual mediante el entorno de ejecución **dotnet**:
   ...
   ```powershell
   func init --worker-runtime dotnetIsolated --force
   ```
...
1. Ahora debería mostrarse un mensaje de error que indica que no se encontró el recurso.
   ...
   > **Nota**: Esto es previsible, ya que el contenedor es privado.
...
1. Reemplace el contenido del código generado automáticamente de la función **FileParser** a partir de la línea 19 por el código siguiente:
