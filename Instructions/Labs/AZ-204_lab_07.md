...
1. En la hoja **Crear aplicación de funciones**, en la pestaña **Aspectos básicos**, realice las siguientes acciones y, a continuación, seleccione **Siguiente: Almacenamiento**:
   ...
   | Lista desplegable de **Hospedaje**        | Seleccione **Consumo (sin servidor)** |
...
1. Si el panel **Configuración** contiene la pestaña **Configuración de la aplicación**, haga clic en ella y realice las siguientes operaciones:

   - Seleccione **+ Nueva configuración de la aplicación**.
   - En la ventana emergente **Agregar o editar configuración de la aplicación**, en el cuadro de texto **Nombre**, escriba **StorageConnectionString**.
   - En el cuadro de texto **Valor**, construya un valor mediante la sintaxis `@Microsoft.KeyVault(SecretUri=<Secret Identifier>)`, donde el marcador de posición `<Secret Identifier>` representa el identificador secreto que registró anteriormente en este ejercicio.
   ...
   - Deje la casilla **configuración de la ranura de implementación** establecida en su valor predeterminado (no seleccionado) y, a continuación, seleccione **Aceptar** para cerrar la ventana emergente y volver a la sección **Configuración**.
   - Seleccione **Guardar** para guardar la configuración y, a continuación, en el cuadro de diálogo emergente **Guardar cambios**, seleccione **Continuar**.

   > **Nota**: Espere a que se guarde la configuración de la aplicación antes de continuar con el laboratorio.

1. Si el panel **Configuración** no contiene la pestaña **Configuración de la aplicación**, seleccione la opción **Variables de entorno** en la sección **Configuración** y realice las siguientes operaciones:

   - Seleccione la pestaña **Configuración de la aplicación**.
   - Escriba **StorageConnectionString** en el primer cuadro de texto disponible en la columna **Nombre**.
   - En el cuadro de texto disponible de la derecha (en la columna **Valor**), escriba una construcción de valor mediante la sintaxis siguiente: `@Microsoft.KeyVault(SecretUri=<Secret Identifier>)`, donde el marcador de posición `<Secret Identifier>` representa el identificador del secreto que registró anteriormente en este ejercicio.

   > **Nota**: Por ejemplo, si el identificador secreto es `https://securevaultstudent.vault.azure.net/secrets/storagecredentials/17b41386df3e4191b92f089f5efb4cbf`, el valor resultante sería `@Microsoft.KeyVault(SecretUri=https://securevaultstudent.vault.azure.net/secrets/storagecredentials/17b41386df3e4191b92f089f5efb4cbf)`.

   - Deje la casilla **Configuración de ranura de implementación** establecida en su valor predeterminado (no seleccionada) y seleccione **Aplicar** el valor que escriba y haga clic en **Confirmar** para confirmar la operación.
