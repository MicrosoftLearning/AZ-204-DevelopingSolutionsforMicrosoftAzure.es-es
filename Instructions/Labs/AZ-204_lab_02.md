...
1. En la hoja **Crear aplicación de funciones**, en la pestaña **Aspectos básicos**, realice las siguientes acciones y, a continuación, seleccione **Siguiente: Almacenamiento**:
    ...
    | Lista desplegable de **Versión** | Seleccione **8 (LTS) aislado (versión preliminar)** |
...
1. En la línea 17, reemplace `"0 */5 * * * *"` por `"0 */2 * * * *"`` para establecer el intervalo de frecuencia periódica en 2 minutos en lugar de 5 minutos y guardar el cambio.
...
1. Observe la ejecución de la función que se produce aproximadamente cada dos minutos. Cada ejecución de función debe representar un mensaje simple en el registro.
1. Cierre la aplicación que ejecuta **Terminal Windows** actualmente.
...
#### Tarea 3: Escritura de código de función desencadenado por HTTP con entrada de blob
...
1. En el símbolo del sistema, ejecute el comando siguiente para registrar la extensión [Microsoft.Azure.Functions.Worker.Extensions.Storage](https://www.nuget.org/packages/Microsoft.Azure.Functions.Worker.Extensions.Storage/6.2.0):
...
1. En la barra de tareas, vuelva a seleccionar el icono **Terminal Windows** para abrir una nueva instancia de la aplicación **Terminal Windows**.
1. En el símbolo del sistema, ejecute el comando siguiente para probar la llamada a la API de REST **POST** en `http://localhost:7071/api/echo` con el cuerpo de la solicitud HTTP establecido en un valor numérico de **3**:
