...
1. En la hoja **Crear aplicación de funciones**, en la pestaña **Aspectos básicos**, realice las siguientes acciones y, a continuación, seleccione **Siguiente: Almacenamiento**:
...
1. Elimine todo el contenido del archivo **Echo.cs** y agregue el código siguiente. Define una función desencadenada por HTTP denominada "Echo" que toma una solicitud POST y devuelve el cuerpo de la solicitud como OkObjectResult. La interfaz ILogger también se usa para registrar información. Esta función se puede usar para probar las solicitudes y respuestas HTTP.
...
#### Tarea 3: Prueba de la función desencadenada por HTTP mediante httprepl
...
1. En el símbolo del sistema, ejecute el comando siguiente para instalar e iniciar la herramienta **httprepl**, con el establecimiento del identificador uniforme de recursos (URI) base en ``http://localhost:7071``:
...
1. En el símbolo del sistema de la herramienta, ejecute el comando siguiente para ir al directorio de **api** relativo:
...
1. En el símbolo del sistema, ejecute el comando siguiente para usar **Azure Functions Core Tools** para crear una función denominada **Periódica** mediante la plantilla **Desencadenador de temporizador**:
...
1. En el símbolo del sistema abierto, ejecute el comando siguiente para ejecutar el proyecto de aplicación de funciones:
...
#### Tarea 4: Actualización de la configuración de integración de funciones
...
#### Tarea 5: Observación de las ejecuciones de funciones

1. En la barra de tareas, seleccione el icono **Terminal Windows**.

1. Ejecute el comando siguiente para cambiar el directorio actual al directorio **Allfiles (F):\\Allfiles\\Labs\\02\    \Starter\\func**:

    ```powershell
    cd F:\Allfiles\Labs\02\Starter\func
    ```

1. 1. En el símbolo del sistema abierto, ejecute el comando siguiente para ejecutar el proyecto de aplicación de funciones:
...
1. En el símbolo del sistema, ejecute el comando siguiente para usar **Azure Functions Core Tools** para crear una función denominada **GetSettingInfo** mediante la plantilla de **Desencadenador HTTP**:
...
#### Tarea 3: Escritura de código de función desencadenado por HTTP e introducido por blob
...
1. Elimine todo el contenido del archivo **GetSettingInfo.cs** y agregue el código siguiente. Este código devuelve el contenido de un archivo JSON en un contenedor de Azure Blob Storage en respuesta a una solicitud HTTP GET.
...
#### Tarea 4: Registro de extensiones de Azure Storage Blob
...
1. En el símbolo del sistema, ejecute el comando siguiente para registrar la extensión [Microsoft.Azure.WebJobs.Extensions.Storage](https://www.nuget.org/packages/Microsoft.Azure.WebJobs.Extensions.Storage/):
...
#### Tarea 5: Prueba de la función mediante httprepl
...
1. En el símbolo del sistema, ejecute el comando siguiente para iniciar la herramienta **httprepl**, con el establecimiento del identificador uniforme de recursos (URI) base en ``http://localhost:7071``:
...
1. En el símbolo del sistema, ejecute el comando siguiente para publicar el proyecto de aplicación de funciones (reemplace el marcador de posición `<function-app-name>` por el nombre de la aplicación de funciones que creó anteriormente en este laboratorio):
...
5. En la hoja de **Aplicación de funciones**, en la sección **Funciones**, seleccione la opción **Funciones**.
