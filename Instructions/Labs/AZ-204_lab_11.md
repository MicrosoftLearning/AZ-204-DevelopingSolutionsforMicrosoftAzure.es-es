...
1.  En la sección **Configuración**, realice los pasos siguientes:
    ...
    Microsoft actualiza este curso de entrenamiento cuando la comunidad nos alerta de los cambios necesarios.

1.  En la hoja de **App Service**, en la sección **Configuración**, seleccione el vínculo **Propiedades**.

1.  Sin embargo, las actualizaciones en la nube se producen con frecuencia, por lo que es posible que se produzcan cambios en la interfaz de usuario antes de que se actualice este contenido de entrenamiento.
...
1.  Agregue al archivo **appsettings.json** el elemento siguiente justo después del elemento **Registros**; para ello, reemplace el marcador de posición `instrumentation-key` por el valor de la clave de instrumentación de recurso de Application Insights que registró anteriormente en este laboratorio:
...
1.  En la ventana del explorador que se abre, vaya a la dirección URL `https://localhost` y agregue la ruta de acceso relativa **/weatherforecast** de la API web.
    ...
    > **Nota**: La dirección URL completa es `http://localhost:[port-number]/weatherforecast`, donde el marcador de posición `[port-number]` identifica el número de puerto en el que se puede acceder a la aplicación web a través del protocolo HTTPS.
...
### <a name="exercise-4-application-insights-logging-with-net"></a>Ejercicio 4: Registro de Application Insights con .NET

#### <a name="task-1-configure-logging-for-a-net-api-app"></a>Tarea 1: Configuración del registro para una aplicación de API de .NET
...
1.  Revise el contenido del archivo y observe que incluye una inserción de constructores para la interfaz <WeatherForecastController> ILogger genérica.
