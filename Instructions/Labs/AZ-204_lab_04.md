...
1. En la hoja **Selección de la opción de API**, selecciona **Crear** en el cuadro **Azure Cosmos DB for NoSQL**.
...
1. Haz clic en **Tokens de acceso compartido** y genera un token de acceso con la configuración predeterminada haciendo clic en **Generar token y dirección URL de SAS**. Anota el valor del **token de SAS de blobs**, ya que lo necesitarás más adelante.
...
1. Actualiza las propiedades **BlobContainerUrl** y **BlobSASToken** estableciendo su valor en la propiedad **URL** del contenedor de blobs de Azure Storage denominado **imágenes** y el valor del token SAS de blobs que registraste anteriormente en este laboratorio.
