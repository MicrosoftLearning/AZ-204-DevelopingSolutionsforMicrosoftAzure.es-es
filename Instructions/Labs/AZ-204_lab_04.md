...
Por último, establecerá el nivel de coherencia de la instancia de Cosmos DB e implementará una función de Azure para las notificaciones de la fuente de cambios.
...
1. En la hoja **Selección de la opción de API**, selecciona **Crear** en el cuadro **Azure Cosmos DB para NoSQL**.
...
1. De nuevo en la sección **Contenedores**, seleccione el contenedor de **images** recién creado.
...
1. Haz clic en **Tokens de acceso compartido** y genera un token de acceso con la configuración predeterminada haciendo clic en **Generar token y dirección URL de SAS**. Ten en cuenta el valor del **token de SAS de blobs**, ya que lo necesitarás más adelante.
...
1. Desde el símbolo del sistema del terminal, ejecute el siguiente comando para agregar la biblioteca cliente de .NET de Azure Cosmos DB al proyecto abierto actualmente:
   ...
   > **Nota**: Antes de continuar con el siguiente comando, asegúrese de que está situado actualmente en esta ruta de acceso: **(F):\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Upload\\**
...
1. Actualice las propiedades **BlobContainerUrl** y **BlobSASToken** estableciendo su valor en la propiedad **URL** del contenedor de blobs Azure Storage denominado **imágenes** y el valor del token de SAS de blobs que registró anteriormente en este laboratorio.

1. Guarde el archivo **appsettings.json** y ciérrelo.

   > **Nota**: Seleccione **Sobrescribir** si recibió un mensaje sobre que el archivo es de solo lectura.

1. En la ventana **Visual Studio Code**, haga clic con el botón derecho en la carpeta **AdventureWorks.Context** en el panel de navegación y luego seleccione **Abrir en terminal integrado**.
...
1. En el símbolo del sistema del terminal, compruebe que el directorio actual está establecido en **AdventureWorks.Context** (o cámbielo si no es así); a continuación, ejecute el siguiente comando para importar **Microsoft.Azure.Cosmos** desde NuGet:
   ...
   > **Nota**: Antes de continuar con el siguiente comando, asegúrese de que está posicionado actualmente en la siguiente ruta: **(F):\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Context\\**

   ```
   dotnet add package Microsoft.Azure.Cosmos --version 3.28.0
   ```

1. Desde el símbolo del sistema de terminal, ejecute el siguiente comando para compilar el proyecto **AdventureWorks.Context**:
...
1. En la pestaña del editor de código del archivo **AdventureWorksCosmosContext.cs**, agregue el código siguiente:
...
1. En el símbolo del sistema del terminal, ejecute el siguiente comando para ejecutar la aplicación web de ASP.NET:
   ...
   > **Nota**: Antes de continuar con el siguiente comando, asegúrese de que está situado actualmente en esta ruta de acceso: **(F):\\Allfiles\\Labs\\04\\Starter\\AdventureWorks\\AdventureWorks.Web\\**
